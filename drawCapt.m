function [captainGraphics, xNet, yNet] = drawCapt (xCapt, yCapt, thetaCapt, sizeCapt)
  % Get the captain matrix for a captain of dimension sizeCapt
  capt = getCapt(sizeCapt);

  % Rotate the captain from zero heading to heading thetaCapt
  R = getRotation(thetaCapt);
  rotatedCapt = R * capt;

  % Shift the captain from (0, 0) to (xCapt, yCapt)
  ...
  captainGraphics(18) = drawLine(pt19, pt20, "k"); % This might be an additional line to connect the net center
endfunction
